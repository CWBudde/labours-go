...
> The renderer has been merged into the [hercules](https://github.com/cwbudde/hercules) repository: it now lives under `internal/render`, the `labours` binary is built from `cmd/labours`, and `hercules report` renders in-process. Rendering output parity was verified before the merge (byte-identical output on identical inputs).
