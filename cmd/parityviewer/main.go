...
func main() {
	...
	baselineDir := flag.String("baseline-dir", filepath.Join("analysis_results", "reference"), "Baseline PNG directory (used when --parity-dir is not set)")
	artifactDir := flag.String("artifact-dir", filepath.Join("analysis_results", "reference"), "Artifact PNG directory (used when --parity-dir is not set)")
	baselinePrefix := flag.String("baseline-prefix", "python_", "Optional filename prefix to strip from baseline images")
	artifactPrefix := flag.String("artifact-prefix", "go_", "Optional filename prefix to add when locating artifact images")
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		renderPage(w, result, buildViewOptions(*parityDir != "", root, artDir, *artifactPrefix))
	})
...
func buildViewOptions(useParity bool, repoRoot, artifactDir, artifactPrefix string) viewOptions {
	if useParity {
		return viewOptions{
			CanRerender:         false,
			RerenderDisabledMsg: "Re-render is disabled in --parity-dir mode because the button only regenerates analysis_results/reference Go artifacts.",
		}
	}
	defaultArtifactDir := filepath.Join(repoRoot, "analysis_results", "reference")
	if !samePath(defaultArtifactDir, artifactDir) || artifactPrefix != "go_" {
		return viewOptions{
			CanRerender:         false,
			RerenderDisabledMsg: fmt.Sprintf("Re-render is disabled because it only updates %s/go_*.png.", defaultArtifactDir),
		}
	}
	return viewOptions{CanRerender: true}
}

func ensureRerenderSupported(useParity bool, repoRoot, artifactDir, artifactPrefix string) error {
	opts := buildViewOptions(useParity, repoRoot, artifactDir, artifactPrefix)
	if opts.CanRerender {
		return nil
	}
...
func rerenderArtifact(repoRoot, name string) error {
	...
	return runLaboursReferenceUpdate(repoRoot, name)
}
...
func laboursReferenceRecipes() []referenceRecipe {
	return []referenceRecipe{
		...
		{
			Name:          "ownership_concentration_timeline",
			Input:         filepath.Join("test", "testdata", "hercules", "report_default.pb"),
			Mode:          "ownership-concentration",
			OutputIsDir:   true,
			GeneratedFile: "ownership-concentration_timeline.png",
		},
		{
			Name:          "ownership_concentration_subsystems",
			Input:         filepath.Join("test", "testdata", "hercules", "report_default.pb"),
			Mode:          "ownership-concentration",
			OutputIsDir:   true,
			GeneratedFile: "ownership-concentration_subsystems.png",
		},
		{
			Name:          "knowledge_diffusion_distribution",
			Input:         filepath.Join("test", "testdata", "hercules", "report_default.pb"),
			Mode:          "knowledge-diffusion",
			OutputIsDir:   true,
			GeneratedFile: "knowledge-diffusion_distribution.png",
		},
		{
		...
		{
			Name:          "knowledge_diffusion_lorenz",
			Input:         filepath.Join("test", "testdata", "hercules", "report_default.pb"),
			Mode:          "knowledge-diffusion",
			OutputIsDir:   true,
			GeneratedFile: "knowledge-diffusion_lorenz.png",
		},
		{
			Name:  "hotspot_risk",
			Input: filepath.Join("test", "testdata", "hercules", "report_default.pb"),
...
func loadCases(useParity bool, parityDir, baselineDir, artifactDir, baselineFilePrefix, artifactFilePrefix, nameFilter, namePrefix string) (loadResult, error) {
	...
	return loadCasesFromDirectories(baselineDir, artifactDir, baselineFilePrefix, artifactFilePrefix, nameFilter, namePrefix)
}
...
func loadCasesFromDirectories(baselineDir, artifactDir, baselineFilePrefix, artifactFilePrefix, nameFilter, namePrefix string) (loadResult, error) {
	...
	if baselineFilePrefix != "" {
		baselineGlob = baselineFilePrefix + "*.png"
	}
	baseline, err := filepath.Glob(filepath.Join(baselineDir, baselineGlob))
	if err != nil {
	...
	for _, baselinePath := range baseline {
		baseName := strings.TrimSuffix(filepath.Base(baselinePath), filepath.Ext(baselinePath))
		name := strings.TrimPrefix(baseName, baselineFilePrefix)
...
<title>Labours-Go Parity Viewer</title>
<style>
...
<div class="sticky-header">
  <div class="controls">
    <h1>Labours-Go Parity Viewer</h1>
...
<script>
(function() {
  var viewerStateStorageKey = 'labours-parity-viewer-state-v1';
